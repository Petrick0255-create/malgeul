...
const el={
  ...
  title:$('#meetingTitle'),date:$('#meetingDate'),source:$('#sourceLanguage'),target:$('#targetLanguage'),speakerCount:$('#speakerCount'),interval:$('#translationInterval'),consent:$('#consentCheck'),
...
function lockSetup(locked){[el.title,el.date,el.source,el.target,el.speakerCount,el.interval,el.file].forEach(input=>input.disabled=locked)}
function setRecordingUI(active){
  ...
  el.captureHint.textContent=active?'버튼을 다시 누르면 회의록을 완성합니다.':`${intervalLabel(Number(el.interval.value)||15)}마다 원문과 화자별 번역이 추가됩니다.`;
...
function updateLiveStatus(){if(pendingChunks>0)el.liveStatus.textContent=`${pendingChunks}개 구간을 Gemini가 분석 중이에요`;else if(recording)el.liveStatus.textContent=`듣는 중 · ${intervalLabel()}마다 새 번역을 만듭니다.`}
async function processChunk(blob,offset){
  let file;
  try{
    file=await uploadGeminiFile(blob,`live-${Date.now()}.webm`);
    const segments=await transcribe(file),normalized=segments.map(segment=>({...segment,speaker:normalizeSpeaker(segment.speaker),start:segment.start+offset,end:segment.end+offset})),translations=await translateSegments(normalized);
...
async function finishMeeting(){
  ...
  let minutes;try{minutes=await createBilingualMinutes()}catch(error){toast(`${error.message} 원문과 번역 대화는 저장할 수 있어요.`);minutes={original:emptyMinutes(),translated:emptyMinutes()}}
...
async function processUploadedFile(file){
  ...
  try{await processChunk(file,0);elapsed=Math.ceil(Math.max(...meetingRows.map(row=>row.end),0));let minutes;try{minutes=await createBilingualMinutes()}catch(error){toast(`${error.message} 대화 기록은 저장할 수 있어요.`);minutes={original:emptyMinutes(),translated:emptyMinutes()}}currentData={rows:meetingRows,minutes,duration:elapsed,source:el.source.value,target:el.target.value};clearInterval(progressId);el.progress.style.width='100%';renderResult()}catch(error){clearInterval(progressId);el.processing.hidden=true;el.setup.hidden=false;lockSetup(false);toast(error.message||'파일 처리에 실패했어요.')}finally{finishing=false}
}
